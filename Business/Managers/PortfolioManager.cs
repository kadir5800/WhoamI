    ...
    public class PortfolioManager : BaseManager, IPortfolioManager
    {
        ...
        public async Task<ClientResult<getAllPortfolioResponse>> getAllPortfolio(dataTableRequest request)
        {
            try
            {
                ...
                var sqlQuery = $@"SELECT [t0].* FROM Portfolio AS [t0] LIKE '%{request.SearchValue}%' ORDER BY [t0].[{request.SortColumn}] {request.SortColumnDir} OFFSET {skip} ROWS FETCH NEXT {takeA} ROWS ONLY";
